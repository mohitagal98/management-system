...
<head>
    ...
    <link rel="stylesheet" href="baate/style.css">
    <link rel="stylesheet" href="https://www.w3schools.com/lib/w3.css">
    <link rel="icon" type="image/png" href="baate/favicon.png">
    <script>
        function loadDoc() {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState === 4 && this.status === 200) {
                    document.getElementById("baate").innerHTML = this.responseText;
                }
            };
            xhttp.open("GET", "baate.php", true);
            xhttp.send();
        }
...
<body onload="loadDoc()">
<div class="container">
    <div class="ak"><span><img src="baate/favicon.png"><h2 style="position: relative;
    bottom: 75px;left: 120px">Chating System</h2></span></div>
    <div class="w3-card chat" style="position: relative;bottom: 50px;">
        <div id="chat"></div>
    </div>
    <form style="position: relative;bottom: 50px;" method="post" action="baate_index.php">
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = "insert into baate(name,msg) values('$Name','$msg');";
        $res = mysqli_query($conn, $query);
        if ($res) {
            echo "<audio src='baate/notification.mp3' autoplay></audio>";
        } else {
